...
export function Button({ children, to, variant: type, onClick, disabled = false }: ButtonProps) {
  const base =
  ...
  const styles = {
    ...
    secondary: `inline-block text-sm rounded-full border-2 border-stone-300 font-semibold tracking-wide text-stone-400 uppercase transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-300 focus:ring focus:ring-stone-200 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed focus:text-stone-800 px-4 py-2.5 md:px-6 md:py-3.5`,
  } as const;
